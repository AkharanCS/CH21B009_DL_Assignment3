...
## Logging Table with predictions for 10 datapoints

# Initializing wandb
wandb.init(project="Assignment3_Q4")
...
preds = []
with torch.no_grad():
    src_batch = latin.to(device)
    trg_batch = hindi.to(device)
    ...
    for latin_seq, pred_seq, true_seq in zip(src, pred_ids, trg):
        ...
        latin_texts.append(latin_seq_trimmed)
        labels.append(true_seq_trimmed)
        preds.append(pred_seq_trimmed)


# Converting list of tokens to strings
for i in range(len(preds)):
    for j in range(len(preds[i])):
        preds[i][j] = trg_itos[preds[i][j]]
    preds[i] = "".join(preds[i])

for i in range(len(labels)):
    for j in range(len(labels[i])):
        labels[i][j] = trg_itos[labels[i][j]]
    labels[i] = "".join(labels[i])

for i in range(len(latin_texts)):
    for j in range(len(latin_texts[i])):
        latin_texts[i][j] = src_itos[latin_texts[i][j]]
